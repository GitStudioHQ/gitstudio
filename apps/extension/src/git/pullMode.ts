...
export function settlePullStop(result: {
  ...
  // Files left to resolve — the stop's own, or the ones an earlier stop left
  // behind — make this a pause like every other "git stopped for you": said
  // through notifyPaused, with Resolve Conflicts… opening the Conflicts
  // dashboard (Accept Yours / Theirs, Merge…, Continue / Skip / Abort). Work in
  // the way, or an operation with nothing left unmerged, has nothing to
  // resolve there: the plain warning, and the Changes view, where the
  // operation's own Continue / Abort banner is.
  const conflicted = result.stopped
