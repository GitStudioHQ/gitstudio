...
export class CommitGraphPanel {
  ...
  private async pushRowStats(shas: string[]): Promise<void> {
    ...
    // One `git log --numstat` per chunk answers the whole visible window. The
    // per-sha version spawned two git processes a row, all sixty at once, and
    // capped there — so on a tall window the rows past sixty were never
    // answered, stayed pending in the webview, and kept a blank CHANGES cell
    // for the rest of the session. Chunked, not capped: a chunk's argv stays
    // well inside Windows' 32K limit however tall the window.
    const bySha = new Map<string, RowStat>();
    try {
      for (let i = 0; i < wanted.length; i += STATS_CHUNK) {
        const chunk = wanted.slice(i, i + STATS_CHUNK);
        for (const s of await active.ctx.commitDetails.getCommitStats(chunk)) {
          bySha.set(s.sha, s);
        }
      }
    } catch {
