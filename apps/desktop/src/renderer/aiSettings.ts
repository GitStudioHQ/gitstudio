...
// ── AI Assistant (agent behavior) card ──────────────────────────────────────────

/** Configure the in-app Assistant agent: model, thinking depth, default access. */
export function assistantCard(): HTMLElement {
  const { card, body } = settingsCard("AI Assistant", "hubot");

  const save = async (patch: Record<string, string>): Promise<void> => {
    try {
      await host.invoke("ai:setAgentConfig", patch as never);
    } catch (e) {
      toast(cleanErr(e), "error");
    }
  };
  ...
  const render = async (): Promise<void> => {
    ...
    const sub = el("div", "settings-sub");
    sub.textContent =
      "How the Assistant agent behaves by default. You can still override these per conversation from the Assistant view.";
    body.append(sub);
    ...
    body.append(
      segField("Model", "Which tier of your connected models the Assistant uses.", [
        { id: "fast", label: "Fast" },
        { id: "mid", label: "Balanced" },
        { id: "deep", label: "Deep" },
      ], cfg.model, (v) => void save({ model: v })),
      segField("Thinking", "How much the model reasons before answering. More thinking is slower but deeper.", [
        { id: "off", label: "Off" },
        { id: "auto", label: "Auto" },
        { id: "extended", label: "Extended" },
      ], cfg.thinking, (v) => void save({ thinking: v })),
      segField("Default access", "What a new conversation may do to your repo. Writes always need your approval.", [
        { id: "read", label: "Read-only" },
        { id: "write", label: "Commits" },
        { id: "destructive", label: "Full" },
      ], cfg.permission, (v) => void save({ permission: v })),
    );
  };
...
/** A labeled segmented control row (label + sub + a row of toggle buttons). */
function segField(
  label: string,
  sub: string,
  ...
  const row = el("div", "settings-seg-row");
  const l = el("div", "settings-field-label");
  l.textContent = label;
  const s = el("div", "settings-sub");
  s.textContent = sub;
  const seg = el("div", "settings-seg");
  const btns: HTMLElement[] = [];
  for (const o of options) {
    const b = el("button", "settings-seg-btn" + (current === o.id ? " active" : ""));
